...
test('recommendation audit is deterministic and covers the complete rollable matrix twice', async () => {
  ...
  assert.equal(first.summary.totalCases, 300);
  assert.equal(Object.keys(first.summary.countByWeapon).length, 10);
  ...
  assert.ok(Object.values(first.summary.countByOffense).every((count) => count === 20));
  assert.ok(Object.values(first.summary.countByAscendancy).every((count) => count >= 12));
  assert.equal(first.schemaVersion, 2);
  assert.equal(Object.values(first.summary.solutionClass).reduce((sum, count) => sum + count, 0), 300);
