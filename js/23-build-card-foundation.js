...
function renderSkillGroups(values, face) {
  return `<div class="rc-skill-groups">${(values || []).map((skill) => `
    <div class="rc-skill-group">
      <div class="rc-skill-group__skill">${renderName(skill, face)}</div>
      ${(skill.supports || []).length ? `<div class="rc-skill-group__supports">${skill.supports.map((support) => `<div class="rc-skill-group__support">${renderName(support, face)}</div>`).join('')}</div>` : ''}
    </div>
  `).join('')}</div>`;
}
