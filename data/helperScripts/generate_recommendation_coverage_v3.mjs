...
for (const weapon of weapons) for (const offense of offenses) {
  ...
  cells.push({
    ...
    ...(result.supportChains.length ? { supportChainCandidates: result.supportChains.map((entry) => ({
      active: { id: entry.candidate.entity.id, name: entry.candidate.entity.name },
      requiredSupports: entry.proof.supportNames.map((name, index) => ({ name, role: entry.proof.supportRoles[index] })),
      intermediateMechanic: entry.proof.intermediateMechanic,
      evidence: `${entry.proof.relation}:${entry.proof.mechanic}:${entry.proof.confidence}`
    })) } : {}),
    counts: { direct: result.direct.length, carrierBridge: result.bridges.length, supportChain: result.supportChains.length }
  });
  if (result.classification === 'CARRIER_BRIDGE') {
...
const totals = Object.fromEntries(['DIRECT', 'CARRIER_BRIDGE', 'SUPPORT_CHAIN', 'GAP']
