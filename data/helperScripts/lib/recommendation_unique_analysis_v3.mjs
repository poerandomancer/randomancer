...
function evidenceRecord(fact, offense, sourceType, sourceName, parentId, component = null) {
  ...
  return {
    ...
    provenance: { parentUniqueId: parentId, sourceType, sourceName,
...
function typedModifierFacts(lines, offense) {
  ...
  return arr(lines).flatMap((line) => {
    ...
    else if (/against|while|if |consum/i.test(line)) relation = 'requires';
    if (!relation) return [];
...
function analyzeWholeUnique(entity, offense, sources = {}) {
  ...
  const records = [];
  const add = (fact, type, name, component) => {
    const record = evidenceRecord(fact, offense, type, name, entity.id, component);
    if (record) records.push(record);
  };
  for (const fact of arr(entity.facts)) add(fact, 'item_fact', entity.name);
  for (const fact of typedModifierFacts([...arr(raw.implicit_mods), ...arr(raw.explicit_mods)], offense)) add(fact, 'item_modifier', entity.name);
  ...
  for (const name of [...grantedNames].sort()) {
    for (const granted of arr(sources.entitiesByName?.get(name))) {
      for (const fact of arr(granted.facts)) add(fact, 'granted_skill', granted.name);
      for (const effect of arr(granted.source_evidence?.granted_effects)) {
        for (const fact of arr(effect.facts)) add(fact, 'granted_effect', granted.name);
      }
      for (const component of arr(granted.components)) for (const fact of arr(component.facts)) add(fact, 'nested_component', granted.name, component.name || component.id);
    }
  }
...
export { FAMILY_MAP, TIERS, analyzeWholeUnique, classifyEmpty, familyOf, legalForWeapon,
  normalizeWeapon, qualityBand, rankCandidates, typedModifierFacts };
