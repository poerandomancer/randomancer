...
function buildStackSkillCard(card, rolledProfile){
  ...
  const attrClass = dominantAttr(g.requirement_weights || g.attributes || {});
  if (attrClass) cardEl.classList.add(attrClass);
  ...
  const requiresSubtitle = (Array.isArray(g.required_weapon_types) && g.required_weapon_types.length)
    ? `<div class="skill-subtitle">${g.required_weapon_types.map(x => x[0].toUpperCase() + x.slice(1)).join(', ')}</div>`
    : '';
  const supportsMarkup = renderSupportCards(supportList, STACK_STATE.gemDict || {});
  ...
  cardEl.innerHTML = `
    ...
    <div class="supports-label">Recommended Supports</div>
    <div class="supports">${supportsMarkup || '<div class="support-item"><div class="support-title">(No supports)</div></div>'}</div>
  `;
...
function renderRecommendationStack(){
  ...
  for (let layer = visible - 1; layer >= 0; layer--) {
    ...
    el.style.opacity = layer === 0 ? 1 : Math.max(0.82, 1 - layer * 0.06);
...
function shuffleRecommendationStack(){
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      ghost.style.transform = `${startTransform} translate(10px, 28px) rotate(-10deg) scale(0.96)`;
    ...
    const slider = document.getElementById('cohesionRange');
	  ...
	  const applyThreshold = (t) => {
		...
		applyThreshold(initialThreshold);
          ...
          const initialView = (document.body?.dataset?.recView) || (window.App && window.App.state && window.App.state.recommendationView) || 'stacked';
...
function ensureUniqueSection(){
  ...
  function renderUniques(items, rolledSet){
          const viewMode = (document.body?.dataset?.recView) || 'stacked';
          STACK_STATE.lastUniques = items;
          if (viewMode === 'stacked') {
                refreshRecommendationStack({ uniques: items });
                return;
          }
