...
(function(){
  ...
  function renderSkillsFromSnapshot(snap){
    ...
    const resolveGem = (entry) => {
      const normalized = (entry && typeof entry === 'object') ? entry : { name: entry };
      const tries = [
        normalized,
        normalized.id,
        normalized.name,
        normalized.base_item?.id,
        normalized.base_item?.display_name
      ].filter(Boolean);
      for (const t of tries) {
        const found = lookupGem(gemDict, t);
    ...
    if (snap.recommendedPersistentBuff) {
      const buffGem = resolveGem(snap.recommendedPersistentBuff);
      if (buffGem) {
