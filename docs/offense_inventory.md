...
`data/offense-inventory.json` is the sole vocabulary for the standard Build draw's Offense axis. A draw chooses one or two distinct rollable entries and may contain at most one `Archetype`. `Critical Hits` remains useful recommendation metadata but is not independently drawable.
